...
export default function DemoLayout(props: { [x: string]: any }) {
  ...
  React.useEffect(() => {
    console.log('🏗️ DemoLayout useEffect - demoData:', demoData ? 'loaded' : 'null');
    console.log('📍 Current location:', location.pathname);
    if (demoData) {
      console.log('✅ DemoLayout: Setting demo data in demoApi');
      console.log('📊 Demo data summary:', {
        users: demoData.users?.length || 0,
        fillials: demoData.fillials?.length || 0,
        applications: demoData.applications?.length || 0,
        merchants: demoData.merchants?.length || 0,
        agents: demoData.agents?.length || 0,
        admins: demoData.admins?.length || 0,
      });
      setDemoData(demoData);
      // Set a fake token for demo mode to avoid "Token topilmadi" errors
      localStorage.setItem('token', 'demo-mode-fake-token');
      console.log('🔑 Demo token set in localStorage');
    }
  }, [demoData, location.pathname]);
